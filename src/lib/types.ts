...
export interface ProspectRow {
  sourceKey: SourceKey; sourceLabel: string; rowNumber: number; mls: string; address: string; city: string; state: string; zipcode: string; county: string; currentPrice: string; listingStatus: string; ownerRaw: string; firstName: string; phone: string; emails: string[]; firstContact: string; status2: string; dripStep: number; nextSendAt: Date | null; lastSentAt: Date | null; stoppedRaw: string; variant: string; malformedDateCount: number;
}
...
export interface RedactedLead {
  id:string; firstName:string; address:string; city:string; state:string; listingStatus:string; sourceKey:SourceKey; sourceLabel:string; dripStep:number; lastSentAt:string|null; nextSendAt:string|null; stopped:boolean; outcome:string; variant:string; dueNow:boolean; completed:boolean; missingDetails:boolean; missingFields:MissingField[];
}
export interface FullLead extends RedactedLead { mls:string; zipcode:string; county:string; currentPrice:string; ownerRaw:string; phone:string; emails:string[]; firstContact:string; }
export interface DashboardData {
  fetchedAt:string; timezone:string;
  headline:{ totalProspects:number; withEmail:number; activeSequences:number; dueNow:number; sentToday:number; sentThisWeek:number; contacted:number; completed:number; stopped:number; knownReplies:number; emailsSent:number; trackedOpens:number; trackedOpenRate:number; notOpened:number; knownReplyRate:number; };
