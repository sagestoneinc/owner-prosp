...
export function mapSheetRows(sourceKey: SourceKey, values: unknown[][], startRow = 2): ProspectRow[] {
  ...
  return values.map((row, index) => {
    ...
    return {
      ...
      mls: cell(row, 0), address: cleanAddress(row[1]), city: cleanAddress(row[2]), state: cleanAddress(row[3]),
      zipcode: cell(row, 4).trim(), county: cleanAddress(row[5]), currentPrice: cell(row, 6).trim(), listingStatus: cleanAddress(row[7]),
      ownerRaw, firstName: displayFirstName(ownerRaw), phone: cell(row, 9).trim(), emails: normalizeEmailCell(row[10]),
      firstContact: cell(row, 11).trim(), status2: cell(row, 12).trim(),
      dripStep: Math.max(0, Math.min(5, Number.parseInt(cell(row, 13), 10) || 0)), nextSendAt: next.date, lastSentAt: last.date,
      stoppedRaw: cell(row, 16).trim(), variant: cell(row, 17).trim().toUpperCase(), malformedDateCount: next.malformed + last.malformed
    } satisfies ProspectRow;
  }).filter(row => Boolean(row.mls || row.address || row.ownerRaw || row.phone || row.emails.length || row.listingStatus || row.dripStep || row.lastSentAt || row.nextSendAt));
}
...
async function googleJson(url: string): Promise<any> {
  ...
  if (!response.ok) {
    let detail = '';
    try { const body = await response.json() as { error?: { status?: string; message?: string } }; detail = [body.error?.status, body.error?.message].filter(Boolean).join(': '); } catch { detail = ''; }
...
export async function fetchAllSourceRows(): Promise<ProspectRow[]> {
  ...
  (Object.keys(SOURCE_TABS) as SourceKey[]).forEach(key => params.append('ranges', `'${SOURCE_TABS[key].title.replace(/'/g, "''")}'!A2:R`));
  params.set('majorDimension', 'ROWS'); params.set('valueRenderOption', 'FORMATTED_VALUE'); params.set('dateTimeRenderOption', 'FORMATTED_STRING');
  const json = await googleJson(`https://sheets.googleapis.com/v4/spreadsheets/${id}/values:batchGet?${params.toString()}`) as { valueRanges?: Array<{ values?: unknown[][] }> };
...
export async function fetchLeadById(id: string): Promise<FullLead | null> {
  const parsed = parseLeadId(id); if (!parsed) return null;
  ...
  const sid = encodeURIComponent(spreadsheetId());
  const json = await googleJson(`https://sheets.googleapis.com/v4/spreadsheets/${sid}/values/${range}?majorDimension=ROWS&valueRenderOption=FORMATTED_VALUE&dateTimeRenderOption=FORMATTED_STRING`) as { values?: unknown[][] };
  const row = mapSheetRows(sourceKey, json.values ?? [], rowNumber)[0]; if (!row) return null;
  return { ...toRedactedLead(row), mls: row.mls, zipcode: row.zipcode, county: row.county, currentPrice: row.currentPrice, ownerRaw: row.ownerRaw, phone: row.phone, emails: row.emails, firstContact: row.firstContact };
}
