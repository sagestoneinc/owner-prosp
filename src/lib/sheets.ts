...
export async function fetchAllSourceRows():Promise<ProspectRow[]> {
  const id=encodeURIComponent(spreadsheetId()); const params=new URLSearchParams(); (Object.keys(SOURCE_TABS) as SourceKey[]).forEach(key=>params.append('ranges',`'${SOURCE_TABS[key].title.replace(/'/g,"''")}'!A2:S`)); params.set('majorDimension','ROWS');params.set('valueRenderOption','FORMATTED_VALUE');params.set('dateTimeRenderOption','FORMATTED_STRING');
