...
function stateLabel(lead:RedactedLead){if(lead.completed)return'Completed';if(lead.stopped)return lead.outcome||'Stopped';if(lead.dueNow)return'Due now';return'Active';}

export default function LeadTable({leads,onOpen}:{leads:RedactedLead[];onOpen:(id:string)=>void}){
  const [filters,setFilters]=useState<LeadFilters>({query:'',source:'all',step:'all',state:'all',details:'all'});
  const filtered=useMemo(()=>filterLeads(leads,filters),[leads,filters]);const visible=filtered.slice(0,250);
  return <section className="panel leads-panel"><div className="panel-header leads-header"><div><div className="section-kicker">LEADS</div><h2>Prospect queue</h2></div><div className="result-count">{filtered.length.toLocaleString()} result{filtered.length===1?'':'s'}</div></div>
    <div className="filters filters-five">
      ...
      <select value={filters.state} onChange={e=>setFilters(f=>({...f,state:e.target.value as LeadFilters['state']}))}><option value="all">All states</option><option value="active">Active</option><option value="due">Due now</option><option value="stopped">Stopped</option><option value="completed">Completed</option></select>
    ...
    <div className="desktop-leads table-wrap"><table><thead><tr><th>Name</th><th>Property</th><th>Source</th><th>Status</th><th>Step</th><th>Last sent</th><th>Next send</th><th/></tr></thead><tbody>{visible.map(lead=><tr key={lead.id}>
      <td><strong>{lead.firstName}</strong>{lead.missingDetails?<small><span className="needs-details">Needs details</span></small>:null}</td><td><strong>{lead.address||'—'}</strong><small>{[lead.city,lead.state].filter(Boolean).join(', ')}</small></td><td><span className={`source-chip source-${lead.sourceKey}`}>{lead.sourceKey}</span></td><td><span className={`status-pill ${lead.dueNow?'status-due':lead.stopped?'status-stopped':lead.completed?'status-complete':''}`}>{stateLabel(lead)}</span></td><td>{lead.dripStep}</td><td>{fmtDate(lead.lastSentAt)}</td><td>{fmtDate(lead.nextSendAt)}</td><td><button className="text-button" onClick={()=>onOpen(lead.id)}>Open</button></td>
    </tr>)}</tbody></table></div>
    <div className="mobile-leads">{visible.map(lead=><button className="mobile-lead-card" key={lead.id} onClick={()=>onOpen(lead.id)}><div><strong>{lead.firstName}</strong><span>{lead.address||'No address'}</span></div><div className="mobile-card-meta"><span className={`source-chip source-${lead.sourceKey}`}>{lead.sourceKey}</span><span className="status-pill">{stateLabel(lead)}</span>{lead.missingDetails?<span className="needs-details">Needs details</span>:null}<span>Step {lead.dripStep}</span></div></button>)}</div>
