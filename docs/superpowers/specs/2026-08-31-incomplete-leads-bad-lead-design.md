...
- Mark Bad Lead — requires confirmation and optional reason.
...
Optional bad-lead reason is not stored separately in the first release to avoid expanding sheet width; the UI may ask for a reason later if a dedicated audit table is introduced.
...
## API
Add authenticated disposition mutation for an existing lead. Recommended shape:
...
A Bad Leads view/filter should allow restoration later; if a full page is excessive, a `Bad leads` filter in the main queue is sufficient for the first release.
